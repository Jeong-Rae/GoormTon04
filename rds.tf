...
# RDS- Replica 서브넷 그룹 생성
resource "aws_db_subnet_group" "rds-replica-subnet-group" {
  name       = "rds-replica-subnet-group"
  subnet_ids = [aws_subnet.db-private-subnet-c.id]

  tags = {
    Name = "RDSReplicaSubnetGroup"
  }
}
...
resource "aws_db_instance" "read-replica-db-instance" {
  ...
  db_subnet_group_name = aws_db_subnet_group.rds-replica-subnet-group.name
  vpc_security_group_ids = [aws_security_group.goorm-sg.id]
